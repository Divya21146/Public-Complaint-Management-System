...
?>
  ...
  $folder="upload/";
   $folder1="images/";
  $qry=mysqli_query($conn,"select * from education where id='$id'");
  $update=mysqli_query($conn,"update education set seen='$seen' where id = '$id'");
  ...
  if(isset($_POST['action']))
{
  ...
  $cseen="not_seen";
  $status="closed";
  $qry3 = mysqli_query($conn, "UPDATE education SET status='$status',cseen='$cseen' WHERE id='$id' ");
$qry7= mysqli_query($conn,"select * from education where id='$id'");
...
if($qry3)
{
header("location:ed_inprogress.php");
}
else
{
