...
$status2="closed";
  ...
  $qry=mysqli_query($conn,"select * from transport WHERE status='$status'");
  if ($qry) {
    $count=mysqli_num_rows($qry);
  }
    $qry1=mysqli_query($conn,"select * from transport WHERE status='$status1'");
    if ($qry1) {
    $count1=mysqli_num_rows($qry1);
  }
  $qry2=mysqli_query($conn,"select * from transport WHERE status='$status2'");
    if ($qry2) {
...
<a href="t_not.php"><div class="card">Not yet processed<br><?php echo $count; ?></div></a>
<a href="t_inprogress.php"><div class="card">In progress<br><?php echo $count1; ?></div></a>
<a href="t_close.php"><div class="card">Closed complaint<br><?php echo $count2; ?></div></a>
</div>
  </div>
